...
def process_incoming_transaction(
    ...
    if referenced_event:
        original_txn = get_transaction_by_referenced_event(referenced_event)
        if original_txn is None:
            raise InvalidRefund(
                f"Invalid refund. Could not find original transaction with {referenced_event}, "
                f"receiver_subaddress: {receiver_subaddr}, and amount: {amount}"
            )
        if (
            original_txn.destination_id == receiver_id
            and original_txn.amount == amount
            and original_txn.currency == currency
            and original_txn.status == TransactionStatus.COMPLETED
        ):
            add_transaction(
                amount=amount,
                currency=currency,
            ...
            raise InvalidRefund(
                f"Invalid refund. referenced_event: {referenced_event}, receiver_subaddress: {receiver_subaddr}, "
                f"amount: {amount}, sequence: {sequence}, blockchain_version: {blockchain_version}"
            )
